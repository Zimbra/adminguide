...
You can also select a specific locale for the {web-app-term} view.
...
User can open the {web-app-term} without having to log on again until the auth token expires.
...
This account is not provisioned on the server, but the external user can sign in to the {web-app-term}, create a display name and set a password to view the shared items.
