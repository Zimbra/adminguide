...
You can also select a specific locale for the {web-app} view.
...
|Auth token lifetime |
Sets a browser cookie that contains the {web-app} auth token.
User can open the {web-app} without having to log on again until the auth token expires.
...
This account is not provisioned on the server, but the external user can sign in to the {web-app}, create a display name and set a password to view the shared items.
