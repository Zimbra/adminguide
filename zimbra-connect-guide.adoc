...
Files shared via Connect will be stored inside a protected root folder the sender's Drive called `Connect sent files`, thus counting towards the sender's mailbox quota, and will remain available for download in the chat window for all participants until deleted from Drive by the sender.
