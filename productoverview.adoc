...
when using the {product-short} {web-client}.

For example, if there is no server connectivity or if server connectivity
is lost, the {web-app} automatically transitions to "`offline mode`".  When server
connectivity is restored, the {web-app} automatically reverts to "`online mode`".
