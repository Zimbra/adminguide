...
IMAP[S]/POP[S]/HTTP[S] client requests to other internal {product-short}
...
IMPORTANT: For {web-client}, Offline mode is no more supported for Chrome versions 85 and above (affects *Kepler9-Patch9* and *Joule8815-Patch16* onwards). Users can still continue to use Offline mode in previous browser versions.

Zimbra Offline Mode allows access to data -- without network connectivity -- when using the {product-short} {modern-client}.
