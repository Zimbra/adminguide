...
The assets required for {product-short} {modern}. This package is automatically installed on each server.
endif::z9[]
...
Users may select the Web Client before they sign in, from the 'Version' drop-down on the login page. The admin can set the Default Web Client to either {web-client} or {modern}, for a COS. Users can override this Default:

* In {modern}, users can go to *Settings->General* to change the value of default Web Client they login to
* In {web-client}, users can go to *Preferences->General->Sign in* to change the value of default Web Client they login to

It is recommended that admins set the Default to {modern}.
endif::z9[]
...
to Microsoft Outlook 2016/2013/2010 with full delegate, offline
