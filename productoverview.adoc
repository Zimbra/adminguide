...
Users may select the Web Client before they sign in, from the 'Version' drop-down on the login page.
The admin can set the Default Web Client to either {web-client} or the {modern-client}, for a COS.
Users can override this Default:

* In the {modern-client}, users can go to *Settings->General* to change the value of default Web Client they login to
* In the {web-client}, users can go to *Preferences->General->Sign in* to change the value of default Web Client they login to
...
ifdef::z9[]
[IMPORTANT]
Currently, the {modern-client} does not support the S/MIME feature.
endif::z9[]
