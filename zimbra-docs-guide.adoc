...
//
Zimbra Docs is based on a heavily customized LibreOffice online (LOOL) package that allows collaborative editing of documents, spreadsheets and presentations straight from the Zimbra WebClient.
...
* Change the btn:[Create] button in the ZWC to the related Docs feature.
...
Before uninstalling the software, the node must be removed from LDAP either from the docs node via command
