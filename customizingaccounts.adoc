...
ifndef::z9[Alerts can be set up to play a sound, highlight the Mail tab when a message arrives, and flash the browser.]
ifdef::z9[Alerts can be set up to play a sound, highlight the Mail tab when a message arrives, and flash the browser, depending on which {web-app-term} they use.]
...
ifdef::z9[(Only the {web-client} can manage this setting currently.)]
...
ifdef::z9[or under *Accounts > Primary account* in the {modern-client}]
...
{prod-short} Contacts allows users to create multiple contact lists and add contact names automatically when mail is received or sent.
...
ifdef::z9[NOTE: Group Calendar features are always available in the {modern-client}.]
| Features
...
ifdef::z9[NOTE: This setting is not provided in the {modern-client}.]

|Show declined meetings |
When enabled, declined appointments display on the {web-client} calendar in a faded view.

[NOTE]
This setting affects the {product} {web-client} only.
...
ifdef::z9[QuickAdd is always available in the {modern-client}.]
...
ifdef::z9[At this time, the {modern-client} supports share management from folder and calendar context menus only.]
...
ifdef::z9[NOTE: The {modern-client} always provides Hi-Def Attachment Previews, with no user settings required.]
