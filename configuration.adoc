...
The {product-short} components are configured during the initial installation of the
...
When {product-short} is configured with more than one server, you can add an additional
...
=== Managing SSL Certificates for {product-short}
...
environments work properly, as the {product-short} system can become non-functional if
certificates are allowed to expire. You can view deployed SSL certificates
from the {product-short} administrator console, including their validation days. It is
...
{product-short}. It is not required for all domains to have DKIM signing enabled for
the feature to work.
...
. Log in to the {product-short} server and as zimbra:
+
[source,bash]
----
...
=== Remove DKIM Signing from {product-short}
...
{product-short} uses SpamAssassin to control spam. SpamAssassin uses predefined rules
...
When {product-short} is installed, the spam/ham cleanup filter is configured on only
the first MTA. The {product-short} spam training tool, *zmtrainsa*, is configured to
...
[NOTE]
--
New installs of {product-short} limit spam/ham training to the first MTA installed. If
...
The {product-short} default is that all users can give feedback when they add or remove
...
messages are received by {product-short}, the {product-short} global rule is to send all messages
...
{product-short} and Microsoft Exchange servers.

{product-short} can query the free/busy schedules of users on supported Microsoft Exchange servers and also can propagate the free/busy schedules of {product-short} users to the Exchange servers.
...
only for {product-short} to Exchange free/busy replication.
...
You can set up free/busy interoperability between {product-short} servers.  Free/Busy
interoperability is configured on each server.

[NOTE]
Each server must be running {product-name} 8.0.x or later.
...
You must have a {product-short} license that is enabled for S/MIME.
