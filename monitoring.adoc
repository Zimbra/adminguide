...
* Log level (e.g. `INFO`)
* Thread number with a prefix (e.g. `[qtp1043351526-547:https:https://localhost:7071/service/admin/soap/DeleteAccountRequest]`, `[Index-9]`, etc)
...
*Thread prefix in mailbox.log*

Thread numbers in mailbox.log are prefixed to identify internal components. Most threads have the following naming convention: "*{thread prefix}-{thread number}*".

Threads with prefix `qtp` are created by Jetty `QueuedThreadPool` and have the following naming convention: "*qtp{hash code}-{thread number}*" where `hashCode` is the hash code value of the instance of `QueuedThreadPool` that owns the thread (see https://docs.oracle.com/javase/8/docs/api/java/lang/Object.html#hashCode()[Object::hashCode] in Java platform documentation).

`\{thread-number\}` in thread names is an integer that monotonously increases within each thread factory. Thread numbers are reset when `mailboxd` process is stopped or restarted.

The following prefixes are currently used for thread names in {product-name}: `btpool`, `pool`, `LmtpServer`, `ImapServer`,`ImapSSLServer`, `Pop3Server`, `Pop3SSLServer`, `ScheduledTask`, `Timer`, `AnonymousIoService`, `CloudRoutingReaderThread`, `GC`, `SocketAcceptor`, `Thread`, `qtp`.
...
image:images/doublesemicolon.png[ua value with double semi-colon]

The following key value pairs are currently supported and may be recorded in log entries in any order and eny combination:
...
* `oproto` - originating protocol of request. This can be passed by internal components that make SOAP requests on behalf of a user (e.g. MTA and IMAPd)
...
image:images/poplogmessage.png[Mailbox Log Entry for POP]

Log records reported by threads that serve SOAP requests will usually contain URL of the request being served, as in the following example:

image:images/zwclogmessage.png[Mailbox Log Entry for SOAP]

The following example shows a record of a failed `IMAP STATUS` request sent by `user1@mydomain.com` using AquaMail mobile app. The user's device has IP address `72.83.144.255` (as reported in `oip` field). The request came to IMAP server via Zimbra nginx proxy, which has IP address `10.4.4.138` (as reported in `ip` and `via` fields).

image:images/imaplogmessage.png[Mailbox Log Entry for IMAP]

The following example shows a record of LMTP server delivering a message. The IP address in this log message most likely belongs to Zimbra MTA running on local network.

image:images/lmtplogmessage.png[Mailbox Log Entry for LMTP]
...
image:images/shortlogmessage.png[Mailbox Log Entry for Purge]
