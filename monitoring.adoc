...
= Monitoring {product-short} Servers
:toc:

The {product-name} ({product-short}) includes the following to
...
=== Tuning Considerations for {product-name} 8.0.3 and later

* *{product-short} Member Servers* -- {product-short} servers under the control of a single
...
{product-short} does not check the log4j changes.  To remove all account loggers and
...
on October 25, 2021, 28 minutes after midnight, a POP3 client with IP address `222.173.186.17` has contacted the {product-name} server and that the request was proxied through a local proxy server with IP `10.1.1.136`.
...
out-of-memory error on June 17, 2021.
...
<<zcs_crontab_jobs,{product-short} Crontab Jobs>>.
...
{product-short} version.  If you do not want to receive an email
...
newer version of ZCO is released before a new version of {product-short}, you can
upload the newer ZCO msi file to the {product-short} server from the Administration
