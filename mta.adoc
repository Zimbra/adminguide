...
{product-short} includes a precompiled version of Postfix to route and relay mail
...
ClamAV software is the virus protection engine enabled for each {product-short}
server.
...
first MTA. The {product-short} spam training tool, `zmtrainsa`, is configured to
automatically retrieve these messages and train the spam filter. The
`zmtrainsa` script empties these mailboxes each day.

[NOTE]
--
New installations of {product-short} limit spam/ham training to the first
...
*Setting Up Trusted Networks:* The {product-short} configuration allows
...
<<monitoring_zcs_servers,Monitoring {product-short} Servers>>.
