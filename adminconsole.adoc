...
The Zimbra Administration Console is a browser-based user interface where you can centrally manage Zimbra servers and user accounts.

== Administrator Accounts

When you log in to the Administration Console, the tasks you are
authorized to perform display on the Navigation pane. These tasks appear as per the rights assigned to the administrator role.

Two types of administrator accounts can be created to manage {product-name}:

Global Administrators:: They have full privileges to manage servers,
global settings, domains, and accounts as well as create other administrators. The software installation creates one global administrator account. You can perform administration tasks from the *Administration
Console* or the command line.

Delegated Administrators:: Global administrators create Delegated Administrators and grant them customized roles to manage different tasks from the *Administration Console*.
...
. On the login screen, enter the complete administrator address - e.g.
*admin@domain.com* - and the password set during
...
Administration Console cannot manage configuration and maintenance tasks -- starting and stopping services and managing
...
configuration page currently in view to view the information in a popup. Guide text is also provided from these popups, as demonstrated in
...
The displays and options in the navigation pane and viewing pane change following your selections. Other portions of the UI -- arrow buttons,
...
specific screens, to enable quick access to functions associated with the
...
The options provided in the *Home* navigation pane are under the *Home* directory. Some of these options lead to
configuration pages; others lead to pages containing reports, as
per your selections.

The illustration on the right is an expanded view of the options currently
supported in the Navigation Pane.

The top bar of the page, currently in view, displays your current position in the hierarchy. You can use multiple options for dismissing the current view:
...
The *Home* screen is the default, login view, which provides the *Home* navigation pane and the Home page. This page provides a snapshot view of
...
provided is as per the number of recipients using either SMTP or LMTP for messaging. The polling intervals for the counts are posted directly beneath each chart.

|Message Volume |
System-wide Information page, for Message Volume, to view charts depicting
the number of recipients of messages-using either SMTP or LMTP-and associated message sizes. These counts are for the last
48 hours, 30, 60, and 365 days. The polling intervals for the counts are posted directly beneath each chart.
...
|*Option* |*Description*
|Accounts (count) |
It is a table of accounts managed by the Collaborator.
...
|Aliases (count) |
It is a  table of Aliases managed by the Collaborator. Each alias is an email
...
It is a  table of Distribution Lists managed by the Collaborator.  A distribution list is a feature that allows you to maintain a list of email addresses and send messages to all of them at once. When you send to a distribution list, you are sending to everyone whose address is in the list. The *To:* address line displays the distribution list address.
...
|Resources (count) |
It is a  table of Resources managed by the Collaborator. A Resource is a location or a piece of equipment that is available when you schedule meetings.
...
*Configure* pages, which enable configurations for individual and
global components.
...
|*Option* |*Description*
|Class of Service |
It displays all the COSs that can be managed from this AdministrationConsole.
--
...
|Domains |
It displays all the domains managed from this Administration Console.
--
* Double-click on a table row to access the configuration screens for the
selected domain,
+
...
|Servers |
It displays all the servers managed from this Administration Console.
--
* Double-click on a table row to access the configuration screens for the
selected server,
+
...
|Admin Extensions |
It displays the Admin Extensions managed from this Administration Console.
--
...
It displays the various rights managed from this Administration Console.
--
...
Global Settings define the default global values for servers, accounts, COS, and domains. If you have not defined the values and parameters explicitly in settings configurations, these default values and parameters apply.

You can change the defaults for Global Settings configured during installation, from Administration Console Global Settings.
...
not take effect until the server restarts.
...
You can configure retention and deletion policies as a global setting or as COS-level policies instead of inheriting from the global
settings.

|Proxy |
Set parameters for Web Proxy, Mail Proxy, or Advanced Proxy parameters.

|S/MIME |
(Secure Multipurpose Internet Mail Extensions): Configure the LDAP settings on the S/MIME tab (if S/MIME feature is enabled). You use LDAP servers to retrieve private keys.
...
| HSM |
(hierarchical storage management): Configure the age of messages before they move to the secondary volume.
...
The *Tools and Migration* screen provides the *Tools and Migration* navigation pane, for access to system software management and system
...
|Downloads |
Access Zimbra utilities, which provides downloadable zip packages - for general administration use, and to synchronize an individual end user - containing migration wizards for various platforms, and Outlook connectors.
...
|Backups |
Access a summary view of current free and total space (MB) based on the most recent system backup. You can
...
associated with a similarly displayed directory path to the backup
target. Additional information is in
...
Get zip files to perform a server-to-server migration of mail, calendar, and contacts from IBM Domino to the {product-name} Server.
...
A text file containing functions you can use to customize the
...
account. However, to return a search from a mailbox ID, the entire ID
...
additional information and access to the configuration for the selected
table entry.
...
|Right-click |
Access the popup menu for a selected table row. The popup menus from the same table may differ from row to row, as demonstrated in the
following examples. |
...
--
* Place a minus sign (-) before the attribute, and enclose an individual message between double quotes to delete it.
...
The following illustration shows a high-level view of the Administration Console UI.
...
navigation pane accessed via the Gear icon or a context menu.

[[using_the_gear_icon]]
==== Using the Gear icon

The *Gear* icon is on the upper right edge of the page
view if related to selectable items in the displayed page.

image::images/administration_console_gear_icon.png[The Gear Icon]

To view the available options, highlight a topic in the navigation pane or the page view; the Gear options not applicable to your selection appear disabled. The following example demonstrates Gear options based on a selected navigation bar topic, versus a table row entry from within
the same page view.
...
|Zimlets |
Deploy, Remove, or Toggle Status
...
[NOTE]
Navigation Pane does not feature the context (popup) menu.
...
Containers in the Administration Console feature a wide range of configuration options.  <<high_level_view_of_administration_console_ui,High-level View of Administration Console UI>> lists the configuration options in these containers.
