...
receive mail.  The account status is displayed on the *Accounts*
...
|Active |
The normal state for a mailbox account.  Mail is delivered and users can log in
...
|Pending |
Assignment for an account when it is created but not yet ready to become
active.  While pending, the login is disabled and messages are bounced.

|Locked |
The user cannot log in but mail continues to be delivered to the account.  The
...
account does not change the account license.

|LockOut |
The automatic state that occurs if the user attempts to log in with an incorrect password.  A LockOut cannot be administratively assigned but will
...
Select an _account_, from the *Gear* icon select *View Mail*.  The user's {web-app-term} account opens in a new browser window.
...
You can add owners to distribution lists and they manage the list from their {web-app-term} account's Address Book, Distribution List folder. 
...
see <<delegated_administration,Delegated Administration>>.
...
Dynamic distribution lists automatically manage their membership.  Users are
...
identify the type of users to be added to the dynamic distribution list.
...
|Member URL |
The Member URL is an LDAP-type URL defining a filter that determines which
users are added to and removed from the list.
