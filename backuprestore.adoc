...
Zimbra mail server. Should only be performed on a running mailbox server.
* `zmrestoreoffline` -- This command restores a full or incremental restore
 to the Zimbra mail server. Should only be performed on a stopped mailbox server.
...
* The `zmrestoreoffline` command is used restore the mail server when
the mail server is stopped. This command is run for disaster recovery.
...
.  Run `zmrestoreoffline` to restore account data from the backed up
sessions.
